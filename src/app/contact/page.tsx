...
const faqs = [
  ...
  { q: 'Can I see your work before committing?', a: 'Absolutely. Book a call and we will walk you through relevant case studies and past projects similar to yours.' },
...
export default function ContactPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // In production, wire this to your email service / API
    setSent(true);
  };
      ...
      <main>
        ...
        <div style={{ padding: '80px 20px', background: 'var(--bg2)' }}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20" style={{ maxWidth: 1180, margin: '0 auto' }}>
            ...
            {/* Form */}
            <div>
              <h2 className="font-syne font-extrabold text-[24px] tracking-[-0.03em] mb-8" style={{ color: 'var(--text)' }}>
              ...
              {sent ? (
                <div className="flex flex-col items-center justify-center text-center p-10 rounded-2xl border gap-4"
                  ...
                  <p className="text-[14px] font-light" style={{ color: 'var(--text-2)' }}>
                    We will get back to you within 48 hours. Check your inbox at {form.email}
                  </p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select value={form.country} onChange={e => setForm({ ...form, country: e.target.value })}
                        className="w-full rounded-xl px-4 py-3 text-[14px] outline-none transition-colors duration-200"
                        style={{ background: 'var(--bg3)', border: '1px solid var(--border)', color: form.country ? 'var(--text)' : 'var(--text-3)' }}>
                    ...
                    <div>
                      ...
                      <select value={form.budget} onChange={e => setForm({ ...form, budget: e.target.value })}
                        className="w-full rounded-xl px-4 py-3 text-[14px] outline-none transition-colors duration-200"
                        style={{ background: 'var(--bg3)', border: '1px solid var(--border)', color: form.budget ? 'var(--text)' : 'var(--text-3)' }}>
                        <option value="">Select budget</option>
                        <option>Under $2,500</option>
                        <option>$2,500 – $8,000</option>
                  ...
                  <div>
                    ...
                    <textarea value={form.message} onChange={e => setForm({ ...form, message: e.target.value })}
                      placeholder="Any details about your timeline, existing systems, or specific requirements..."
                      rows={4}
                  ...
                  <button type="submit"
                    className="inline-flex items-center justify-center gap-2 py-4 rounded-full font-semibold text-[15px] transition-all duration-200 hover:scale-[1.02] mt-2"
            ...
            <div className="flex flex-col gap-8">
              ...
              <div className="p-6 rounded-2xl border" style={{ background: 'var(--bg3)', borderColor: 'var(--border)' }}>
                ...
                <a href="mailto:hello@esperpartners.com" className="text-[15px] font-medium no-underline"
              ...
              <div className="p-6 rounded-2xl border" style={{ background: 'var(--bg3)', borderColor: 'var(--border)' }}>
                ...
                <p className="text-[13px] font-light mt-1" style={{ color: 'var(--text-2)' }}>
                  We respond to every enquiry within 48 business hours with a clear, honest proposal.
                </p>
              </div>
                ...
                <div className="flex flex-col gap-3">
                  {offices.map((o) => (
                    <div key={o.city} className="flex items-center justify-between p-4 rounded-xl border"
                      ...
                      <a href={`mailto:${o.email}`} className="text-[12px] no-underline" style={{ color: 'var(--orange)' }}>
                        {o.email}
                      </a>
                    </div>
                  ))}
