...
export default function Services() {
  ...
  return (
    <section id="features" ref={ref} style={{ padding: '100px 40px', background: 'var(--bg)' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto' }}>
        <div className="reveal"><div className="text-[11px] font-bold tracking-[.14em] uppercase mb-4" style={{ color: 'var(--orange)' }}>What We Build</div></div>
        ...
        {/* Bento grid */}
        <div className="reveal d2 rounded-[20px] overflow-hidden grid" style={{ gap: 2, background: 'var(--border)', gridTemplateColumns: '1fr 1fr 1fr', gridTemplateRows: 'auto auto' }}>

          {/* Tall: Web & Mobile */}
          <div className="bento-card p-9 relative overflow-hidden" style={{ gridRow: 'span 2' }}>
            <div className="text-[11px] font-bold tracking-[.1em] uppercase mb-3" style={{ color: 'var(--orange)' }}>Web &amp; Mobile</div>
            <div className="font-syne font-extrabold text-[22px] leading-[1.2] tracking-[-0.03em] mb-2" style={{ color: 'var(--text)' }}>Apps That Feel Alive</div>
            <p className="text-[14px] font-light leading-[1.65] mb-6" style={{ color: 'var(--text-2)' }}>High-performance web apps and native mobile experiences with buttery-smooth UX your users will love.</p>
            <div className="flex flex-col gap-[10px]">
              {[
                ...
                const Icon = n.icon;
                return (
                  <div key={i} className="flex items-center gap-3 rounded-xl border p-3 text-[13px]"
                    style={{ background: 'var(--bg3)', borderColor: 'var(--border)', animation: `slideInNotif .5s var(--ease) ${i * 0.15}s both` }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-medium" style={{ color: 'var(--text)' }}>{n.text}</div>
                      <div className="text-[11px] mt-[2px]" style={{ color: 'var(--text-2)' }}>{n.sub}</div>
                    </div>
          ...
          {/* Wide: Analytics */}
          <div className="bento-card p-9 relative overflow-hidden" style={{ gridColumn: 'span 2' }}>
            <div className="text-[11px] font-bold tracking-[.1em] uppercase mb-3" style={{ color: 'var(--orange)' }}>Data &amp; Analytics</div>
            <div className="font-syne font-extrabold text-[22px] leading-[1.2] tracking-[-0.03em] mb-2" style={{ color: 'var(--text)' }}>Revenue Intelligence</div>
            ...
            <div className="font-syne font-extrabold text-[28px] tracking-[-0.03em] mt-5" style={{ color: 'var(--text)' }}>$1.26M</div>
          ...
          {/* Enterprise */}
          <div className="bento-card p-9 relative overflow-hidden">
            <div className="text-[11px] font-bold tracking-[.1em] uppercase mb-3" style={{ color: 'var(--orange)' }}>Enterprise</div>
            <div className="font-syne font-extrabold text-[22px] leading-[1.2] tracking-[-0.03em] mb-2" style={{ color: 'var(--text)' }}>Mission-Critical Systems</div>
            <p className="text-[14px] font-light leading-[1.65] mb-5" style={{ color: 'var(--text-2)' }}>ERP, CRM, and custom enterprise platforms built for Fortune-level scale and compliance.</p>
            <div className="flex flex-col gap-2">
              {[{ icon: Lock, label: 'Security Layer', pct: 98 }, { icon: Plug, label: 'API Gateway', pct: 91 }, { icon: Cloud, label: 'Cloud Infra', pct: 87 }].map((s, i) => {
                const Icon = s.icon;
                return (
                  <div key={i} className="flex items-center gap-3 rounded-[10px] border p-3 text-[13px]"
                    ...
                    <div className="w-[26px] h-[26px] rounded-lg flex items-center justify-center flex-shrink-0"
                      style={{ background: 'rgba(255,98,0,0.15)', border: '1px solid rgba(255,98,0,0.2)' }}>
          ...
          <div className="bento-card p-9 relative overflow-hidden">
            ...
            <div className="font-syne font-extrabold text-[22px] leading-[1.2] tracking-[-0.03em] mb-2" style={{ color: 'var(--text)' }}>Intelligent by Default</div>
            <p className="text-[14px] font-light leading-[1.65] mb-5" style={{ color: 'var(--text-2)' }}>LLM integrations, ML pipelines, and automation that transforms how your business operates at scale.</p>
            <div className="flex flex-col gap-2">
              {[{ icon: Bot, label: 'AI Model v3.1', pct: 95 }, { icon: BarChart3, label: 'Automation Engine', pct: 88 }, { icon: Cloud, label: 'Predictions', pct: 78 }].map((s, i) => {
                const Icon = s.icon;
                return (
                  <div key={i} className="flex items-center gap-3 rounded-[10px] border p-3 text-[13px]"
                    style={{ background: 'var(--bg3)', borderColor: 'var(--border)' }}>
